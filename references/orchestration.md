...
Before dispatching any channels, archive role-type directories in `output/verified/` that are NOT in the active `--role-types` list. This is defense in depth against the V22 stale-directory failure where old directories from prior runs caused dedup to incorrectly archive everything.
...
Each dispatch includes HC-10 mandatory variables: `working_dir` (absolute path to the repo root), `output_directory`, `dashboard_url`.
...
- `working_dir` — absolute path to the v26 directory
...
5. **Log dedup actions in session-state.md.** Append dedup summary (total input, archived, remaining) to the session-state checkpoint.

6. **Update state (Step 14).** Sync verified results into state.json:
...
7. **Read delta (Step 15).** Read `output/_delta.json` for `new_jobs`, `still_active`, `expired_count`, `rejected_count`.

8. **Incremental commit (interactive mode only, Step 11b continuation).** After dedup:
...
All dispatch prompts MUST set `working_dir` to the absolute path of the repo root. NEVER use relative paths. Subagent dispatch prompts that contain relative `./` paths are invalid — stop and fix before dispatching.
