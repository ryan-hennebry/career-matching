# Design: Job Search Agent V19

## Context

V18 built successfully (21/21 steps, 100% verification), but the first runtime session exposed 8 failures — all implementation-level. The analysis (jsa-v18-analysis.md) documented 8 specific fixes. The subsequent /research (full) confirmed the V18 codebase already partially addresses 3 of the 8 fixes (dedup uses domain:company:title, API keys pipe via stdin, session-state per-batch is a core rule). User chose to re-validate all 8 fixes rather than only the remaining gaps.

**From V18 research handoff:**
...
- Open questions: (1) Pre-populate agent-memory from regressions? (2) Vercel Hobby 10s timeout sufficient?

**From decision log (V18):**
- V18 chose three-phase sequential (Infrastructure → Backend → Frontend) — appropriate for V18 which mixed infrastructure, backend, and frontend changes
...
All 8 fixes land in one phase because they share the same failure domain (constraint enforcement) and the same debugging workflow (read the diff, check the text). V18's three-phase split was justified by three distinct failure domains (infrastructure, backend, frontend). V19 has one domain.
