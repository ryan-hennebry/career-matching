...
class TestGitHubWorkflowUsesClaude:
    ...
    def test_is_repo_root_relative(self):
        """Standalone repo: workflow must be root-relative, with no monorepo
        version-directory paths (tests/vNN/...)."""
        text = _workflow_text()
        for line in text.splitlines():
            stripped = line.strip()
            if stripped.startswith("#"):
                continue
            for old in ("v20", "v21", "v22", "v23", "v24", "v25", "v26"):
                if f"tests/{old}" in stripped:
                    pytest.fail(
                        f"Active (non-comment) line references monorepo version dir {old}: {stripped}"
                    )
